...
to `mean(age)`, `mean(age)` to `mean`, and `mean(age)` to
`mean(income)`. Changing only `d=` also preserves the filter. A filter
starting on a mean continues through later counts; a subsequent change
from a non-mean to a mean stops it unless another explicit filter starts
there.
...
`mean`, `mean(variable)`, their `mean_unw` variants, and the legacy
`Mean` form, with or without `d=`. A blank statistic cell does not
itself mean "stop the filter." Changes such as `p` to `n`, `n` to
`n_unw`, `mean` to `mean(age)`, `mean(age)` to `mean(income)`, and
`mean(age)` to `n` all preserve it. Only entering a mean from a non-mean
stops an inherited additional filter.
...
code 1 within that base. Keep the complete column condition on one
logical line; when a cell contains several instruction lines, the
column parser takes the last remaining condition line. A blank column
instruction can inherit the preceding instruction, so write changed
conditions explicitly. See [worksheet filters](#worksheet-filters) for
how far an additional filter extends.
...
survey. Put the filter on the mean column itself: entering a mean can
stop an inherited additional filter, as explained above.
