...
micsPlusTableR is an R package for preparing, tabulating, reviewing, and
exploring MICS Plus survey data. It provides a browser-based application
and functions that can be used directly in R. R performs the
calculations; the application provides controls for selecting inputs,
running analyses, and examining results.
...
instructions for the tasks they need. Plan authors should also use the
[worksheet reference](#worksheet-reference) for filters and display
settings. Reviewers can focus on the plan's definitions, calculated
results, consistency checks, and exported tables. Users maintaining
preparation scripts or running the workflow directly in R can use the
package reference linked under [Terms and reference
information](#guide-terms). One user may take several of these roles.
...
3.  [**Install the package and start the app**](#install-and-start) -
    install micsPlusTableR in R, then open the app in your web browser.
...
10. [**Download a data file**](#table-data) - combine calculated table
    cells and download them as Excel, CSV, or RDS.
11. [**Look at prepared data**](#view-data) - inspect selected household
    or household-member variables in Data View.
12. [**Explore a question**](#explore-data) - examine frequencies or
...
- [**Worksheet reference for plan authors**](#worksheet-reference) -
  understand filter scope and choose decimal places.
...
![](images/clipboard-2782092616.png)
...
the color. The **Difference** column shows zero where the compared
values agree. **Sum of group values** is compared with the **Reference
total**; in column percentage checks, **Sum of percentages** is compared
with 100. **Worksheet row** and **Worksheet column** identify positions
in the tabulation plan. The screenshot shows the older column labels;
`diff_value` is now displayed as **Difference**.
...
form. You can inspect tables in **Tabulator** without creating these
files.
...
1.  First [create your workbook files](#save-excel) in **Write to
    Excel**, if you have not already done so for this run.
...
# 10. Download a data file {#table-data}
...
# 12. Explore a question {#explore-data}
...
5.  Leave the filter box blank for all prepared records, or enter the
    condition for the population you want to examine. Click **Run Data
    Explorer** and read the result.

**Check:** the selected data, variables, and weighting match the
question you are exploring. These choices affect the meaning of a
result. Apply your survey team's review process before reporting a
result as a final survey finding.
...
create new workbook files if you intend to export tables. To continue an
existing workbook, select it using the **Choose existing...** buttons in
**Write to Excel**. Writing the same sheet again replaces its previously
written values.
...
Use this reference when creating, adapting, or reviewing a tabulation
plan. It explains worksheet filters and decimal settings. Users running
an agreed plan can use the app's tools without changing these
instructions. After editing a plan, save and upload the updated
workbook, then rerun the affected tables and checks.

## Understand worksheet filters {#worksheet-filters}

This section is for people who prepare or review the tabulation plan. It
...
- **A non-mean statistic changes to a mean.** For example, when `n` is
  followed by `mean(age)`, that mean cell and following cells use B
  alone. Returning to `n` does not restart the old filter. This rule is
  checked across columns separately for each row; changes down rows do
  not stop it.
...
the worksheet reader fills missing statistic cells and ignores
surrounding spaces when identifying means. Supported mean forms are
`mean(variable)`, `mean`, and the legacy `Mean` form. A blank statistic
cell does not itself mean "stop the filter." Changes such as `p` to `n`,
`n` to `n_unw`, `mean(age)` to `mean(income)`, and `mean(age)` to `n`
all preserve it. Only entering a mean from a non-mean stops an inherited
additional filter.
...
Only filter inheritance stops on entering a mean from a non-mean.
Existing data-source, weight, calculation, and row/column-condition
instructions keep their roles. A calculation-only instruction does not
end the filter's column range.
...
at every statistic change, including `p` to `n`; this could give
percentages and their counts different populations. Recalculate affected
tables and review denominators and suppression. B remains global,
including in later blocks; only non-mean-to-mean transitions now stop
inherited filters.
...
This section is for people who prepare or review the tabulation plan.
Add **`d=` inside a statistic's parentheses** to choose how many decimal
