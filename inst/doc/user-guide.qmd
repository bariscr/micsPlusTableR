...
Within a horizontal filter block, the new variable is available from that
column onward. A later mutation can use it or redefine it for later columns;
earlier results keep the earlier definition. Multiline and multiple mutation
instructions are supported. Filters run before these calculations. Use the
preparation script for variables needed by filters or across filter blocks.
Worksheet mutations use temporary data and do not change prepared `hh`/`hl`.
