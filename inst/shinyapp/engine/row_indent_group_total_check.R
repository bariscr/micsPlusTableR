...
row_indent_group_total_check <- function(cell_results, diff = 1e-11) {
  ...
  checker_n2 <- cell_results |> filter(stat_type %in% c("n", "n(100)", "n_unw", "n_unw(100)")) |> nrow()

  checker <- checker_n > 0 & checker_n2 > 0

  if (isFALSE(checker)) {
    cat("There isn't a rowwise indent total in this table.")
    totals_indent_row_df <<- NULL
    totals_indent_row_df_issue <<- NULL
    totals_indent_row_df_issue_n <<- 0
  ...
  totals_indent_row_df <<- totals_indent_row_df
  
  totals_indent_row_df_issue <-
    totals_indent_row_df |>
    filter(!is.na(total_value), abs(diff_value) > diff)
  
  totals_indent_row_df_issue_n <<- nrow(totals_indent_row_df_issue)
