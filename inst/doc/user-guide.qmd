...
instructions for the tasks they need. Anyone who wants to understand
how a table is created can use [Understand the tabulation
plan](#worksheet-reference), whether reading an existing plan or editing
one. Reviewers can focus on the plan's definitions, calculated
...
- [**Understand the tabulation plan**](#worksheet-reference) - locate
  datasets, interpret filters and conditions, and read the statistics
  and display settings that produce each table.
...
dataset, understand the filters, then read the conditions and statistics.
The headings describe the results; the instructions determine the values.

## Find the dataset name {#worksheet-dataset}

Look in **column B, rows 4 to 7** for the cell containing a name ending
in **`.sav`**, such as `hh.sav` or `hl.sav`. The app uses that cell's row
as the worksheet's **condition row**, where column-level instructions
are recorded. Its position can vary with the number of heading rows.
...
Here, `hh` is the dataset, the filter restricts the population to adults,
and `w` is the weight variable. The names are illustrative; read the
actual names in your plan. In a horizontal table, a later instruction
block may specify a different dataset. A blank source or weight entry
inherits the preceding setting, so check earlier instruction cells too.
...
- **A non-mean statistic changes to a mean.** This applies to both bare
  `mean` and `mean(variable)`: for example, `n` followed by `mean`, or
  `p` followed by `mean(HCS8)`. That mean cell and following cells use B
...
Changes among non-mean statistics, between consecutive mean types, and
from a mean to a count do **not** stop the filter. This includes `mean`
to `mean(age)`, `mean(age)` to `mean`, and `mean(age)` to `mean(income)`.
Changing only `d=` also preserves the filter. A filter starting on a mean
...
| G | `mean` | None | All adults: entered a mean from a non-mean |
...
and does not include D's sex restriction. Other row
...
`mean`, `mean(variable)`, their `mean_unw` variants, and the legacy `Mean`
form, with or without `d=`. A blank statistic cell does not itself mean
"stop the filter." Changes such as `p` to `n`, `n` to `n_unw`, `mean` to
`mean(age)`, `mean(age)` to `mean(income)`, and `mean(age)` to `n` all
preserve it. Only entering a mean from a non-mean stops an inherited
additional filter.
...
## Read the row and column conditions {#worksheet-conditions}

**Row conditions** are in column B below the condition row, beside the
labels in column A. For example, a row labelled "Urban" might use
`area == 1`. Read the expression and the prepared variable's value
labels together to confirm what the row includes.

**Column conditions** are in column C onward on the condition row,
below the column headings. For example, `HCS7 == 1` identifies records
with that response. A cell containing a filter and a column condition
can separate them with `- - -`:
...
The first line restricts the available population to adults. The last
line defines the outcome for this column. They have different roles:
moving `HCS7 == 1` into the filter would also restrict the percentage's
denominator and could change the meaning of the result.

**How they work together.** In a horizontal table, the active filters
first define the available records. The row condition selects a group
within those records. For a percentage or bare `mean`, the column
condition identifies the outcome within that row's population. For a
count, it identifies the records to count. For `mean(variable)`, it
selects the records whose named variable is averaged. In a vertical
table, column conditions define the populations and row conditions
define the outcomes within them.

For example, with B's `filter(age >= 18)`, a row condition `area == 1`,
a column condition `HCS7 == 1`, and statistic `p`, a horizontal table
shows the percentage meeting `HCS7 == 1` among adults with `area == 1`,
...
worksheet reader fills blank statistic cells from preceding instructions;
a blank cell does not necessarily mean "skip this cell" or "show zero."
Read the applicable statistic together with the dataset, filters, weight,
and row and column conditions.
...
For a horizontal `n_unw` column using the special condition `hhmembers`,
the app sums `HLnum` for qualifying household records. This gives a
member count rather than a count of household records.
...
**Mean types and filters.** Bare `mean`, `mean(variable)`, and their
unweighted forms all count as mean types. Entering one from a non-mean
statistic stops an inherited additional horizontal filter, unless a new
explicit filter starts there. Consecutive mean columns retain their
active filter, including changes of variable, form, or `d=`. Column B's
global filter remains active.
...
adding a decimal argument does not make an unsupported form available
...
Use the ordinary `n`, `n_unw`, and `p` forms unless an existing plan needs
its variants. Equal count calculations do not imply identical handling
elsewhere: automatic direction, count linkage, suppression, and checks
can depend on the exact statistic name and its worksheet position.
...
plan, save and upload it again, then rerun the affected tables and checks.
...
plan, add **`d=` inside a statistic's parentheses** to choose how many decimal
